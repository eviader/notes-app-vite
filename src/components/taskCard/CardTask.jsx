...
function CardTask() {
  ...
  useEffect(()=>{
    setTimeout(()=>{
       if (notes != undefined){ setLoad(false) } 
    },2000)
  },[])
  
  function handleEditNotes(id) {
    editNotes(id, titleValue, textValue)
    setTitleValue("")
    setTextValue("")
  }
            ...
            <main className='container-main'>
              <section className='container-nav'>
                <div className='container-tools'>
                  <button className='save-button' onClick={() => {handleEditNotes(n.id), setLoad(false)}} >{n.upCloud ?  <IconCloudCheck /> : <IconCloudUp /> }</button>
              ...
              <section className='container-text'>
                <textarea className='title-cards' placeholder='Asunto'  onChange={(eTitle) => setTitleValue(eTitle.target.value)}>{n.title}</textarea>
                <textarea className='input-cards' onMouseEnter={()=> handlerId(n.id)}  placeholder='Escribe tu tarea' style={n.styleText} onChange={(eText) => setTextValue(eText.target.value)}>{n.note}</textarea>
              </section>
            </main>
          </div>
